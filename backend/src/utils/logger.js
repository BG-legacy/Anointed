...
const logger = pino({
  // logger is a pino instance that is used to log the messages to the console
  level: config.logLevel || 'info', // the level of the logger
  transport:
    process.env.NODE_ENV !== 'production'
      ? {
          // if the environment is not production, use the pino-pretty transport
          ...
          options: {
            // the reason why i created this is because i want to use the pino-pretty transport to log the messages to the console
  ...
  formatters: {
    // the formatters option is used to format the messages to the console
    level: (label) => {
      // the level option is used to format the messages to the console
