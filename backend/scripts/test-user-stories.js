...
async function testUserStories() {
  ...
  try {
    ...
    console.log(`  INFO - Welcome: ${authUser.first_name} ${authUser.last_name}`);
    ...
    console.log(`  PASSED - Wrong password rejected: ${!wrongPasswordResult.rows[0].password_valid}`);
    ...
    console.log(`  PASSED - Session validation: ${validateSessionResult.rows.length > 0 ? 'VALID' : 'INVALID'}`);
    if (validateSessionResult.rows.length > 0) {
      const validSession = validateSessionResult.rows[0];
      console.log(`  INFO - Session user: ${validSession.first_name} ${validSession.last_name}`);
    }
    ...
    console.log(`  PASSED - Exact search for 'Alice': Found ${exactSearchResult.rows.length} users`);
    for (const user of exactSearchResult.rows) {
      console.log(`    RESULT - ${user.first_name} ${user.last_name} (${user.email})`);
    }
    ...
    console.log(`  PASSED - Fuzzy search for 'Alise' (misspelled): Found ${fuzzySearchResult.rows.length} similar users`);
    for (const user of fuzzySearchResult.rows) {
      console.log(`    RESULT - ${user.first_name} ${user.last_name} (similarity: ${user.name_similarity.toFixed(3)})`);
    }
    ...
    console.log(`  PASSED - Domain search '@anointed.com': Found ${domainSearchResult.rows[0].user_count} users`);
    ...
    console.log(`  INFO - New name: ${updatedUser.first_name} ${updatedUser.last_name}`);
    ...
    console.log(`  INFO - Total sessions: ${totalSessionsResult.rows[0].total}`);
    console.log(`  INFO - Active sessions: ${activeSessionsResult.rows[0].active}`);
    ...
    console.log(`  PASSED - Cleaned up ${cleanupResult.rows.length} expired sessions`);
    ...
    console.log(`  PASSED - Full-text search for 'builder': Found ${fullTextResult.rows.length} matches`);
    for (const user of fullTextResult.rows) {
      console.log(`    RESULT - ${user.first_name} ${user.last_name} (rank: ${user.rank.toFixed(3)})`);
    }
    ...
    console.log('  PASSED - User Registration - Users can register with encrypted passwords');
    console.log('  PASSED - User Authentication - Users can login with proper validation');
    console.log('  PASSED - Session Management - Sessions are created, validated, and cleaned up');
    console.log('  PASSED - Fuzzy Search - Trigram search finds similar names');
    console.log('  PASSED - Profile Management - Users can update their information');
